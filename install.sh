...
# @@File             :  install.sh
# @@Description      :  Install configurations for qterminal
...
__run_pre_install() {
  ...
  if [ -d "$HOME/.config/qterminal.org" ]; then
    if [ ! -f "$HOME/.config/qterminal.org/.installed" ]; then
      backupapp "$HOME/.config/qterminal.org" "$APPNAME" &>/dev/null
      __rm_rf "$HOME/.config/qterminal.org"
    fi
  fi
  return $getRunStatus
}
...
__run_post_install() {
  ...
  if [ ! -f "$HOME/.config/qterminal.org/.installed" ]; then
    ...
    __symlink "$APPDIR/qterminal.org.ini" "$APPDIR/qterminal.ini"
    __replace_one replacehome "$HOME" "$APPDIR/qterminal.org.ini"
    __mv_f "$APPDIR" "$HOME/.config/qterminal.org"
  fi
  touch "$HOME/.config/qterminal.org/qterminal_bookmarks.xml"
  return $getRunStatus
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
